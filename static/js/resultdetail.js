...
app.controller("resultdetailCtrl", function($scope, $http, $location) {
	...
	.then(function(data) {
		...
		var query_condition = ["AND", "resultdetails.uid", operators[0], angular.copy(id)]; // resultdetails.uid
		...
		$http({
			...
			id = $scope.resultdetail["resultdetails.sample_id"];
			...
			$http({
				...
				id = $scope.result["results.sample_id"]; // samples.sample_id
				...
				$http({
					...
					$http({
						...
						id = $scope.sample["samples.sample_id"];
