...
app.controller("patientCtrl", function($scope, $http, $location) {
	...
	.then(function(data) {
		...
		var query_condition = ["AND", ids["patients"], operators[0], angular.copy(id)]; // patients.mrn
		...
		$http({
			...
			id = $scope.patient[angular.copy("patients.sample_id")];
			...
			$http({
				...
				for (let a = 0; a < queryResults.length; a++) {
					samples = samples.concat(queryResults[a]["samples.sample_id"]);
				}
				...
				for (let a = 0; a < samples.length; a++) {
					query_condition = ["AND", "experiments.sample_id", operators[0], angular.copy(samples[a])];
					selected_conditions = [angular.copy(query_condition)];
					queryResults = [];
